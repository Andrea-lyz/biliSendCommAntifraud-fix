...
>
> 首次安装/升级后先打开模块 App，在菜单“API 102 框架状态”检查连接、作用域和配置同步。未连接框架时仍可使用手动检查。旧框架重定向目录中的设置不会自动迁出，可用“导入旧版设置”恢复自行导出的 config.xml；账号和历史数据库不变。GitHub Actions 会打 R8 Release 包并用 Android 默认 debug 签名（androiddebugkey）发布到 Releases；可覆盖本仓库 CI/本地默认签名包，不能覆盖原作者正式签名包。推送 v* 标签或在 Actions 里手动跑 Release 工作流即可出包。
